...
# 调登录接口
def GetBrokerInfoapi(url,payload):
...
# 接口测试脚本
def testGetBrokerInfoApi(dataSourceSheetObj):
    try:
        ...
        for idx, data in enumerate(dataIsExcuteColumn[1:]):
            ...
            if data.value == "y":
                requiredDatas += 1
                logging.info('开始执行【获取经纪人信息API】测试用例')
                # =============================================
                ...
                try:
                    # 注册============改文字======
                    #print("开始执行注册接口")
                    returnMessage = str(GetBrokerInfoapi(url,payload))
                except Exception as e:
                    logging.debug("执行【获取经纪人信息API】发生异常",traceback.format_exc())
                    # 写入失败信息
