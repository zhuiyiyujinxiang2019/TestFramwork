...
# 接口测试脚本
def testResetPwApi(dataSourceSheetObj):
    try:
        ...
        for idx, data in enumerate(dataIsExcuteColumn[1:]):
            ...
            if data.value == "y":
                requiredDatas += 1
                logging.info('开始执行【忘记密码API】测试用例')
                # =================================改================
                ...
                try:
                    # 注册============改======
                    #print("开始执行注册接口")
                    returnMessage = str(reset_pw_api(url,payload))
                except Exception as e:
                    logging.debug("执行【忘记密码API】发生异常",traceback.format_exc())
                    # 写入失败信息
